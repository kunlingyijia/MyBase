...
{
    CGFloat size = kNodeSize * 3 + kNodeInterval * 4;
    ...
    /*
     线
     */
    _lineColor = [UIColor purpleColor].CGColor;
    /*
     九个点
     */
    ...
    for (NSInteger row = 0; row < 3; row ++)
    {
        ...
        {
            CGRect frame = CGRectMake(col * (btnW + space), row * (btnW + space), btnW, btnW);
            JXNodeModel *node = [[JXNodeModel alloc] init];
            node.frame = frame;
            node.normalImage = [UIImage imageNamed:@"密码1"];
            node.selectedImage = [UIImage imageNamed:@"密码3"];
            node.errorSelectedImage = [UIImage imageNamed:@"密码2"];
            node.index = index + 1;
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetRGBStrokeColor(context, 242/255.0, 48/255.0, 48/255.0, 1.0);
